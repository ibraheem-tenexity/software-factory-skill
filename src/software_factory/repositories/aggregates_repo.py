"""Cross-run aggregate reads (SQLAlchemy Core) — direct reads over the flat `agents`/`tickets`
...
from ..models import agents, tickets
...
class AggregatesRepository:
    ...
    def agent_rollups(self) -> list:
        ...
        stmt = (select(
                    agents.c.role,
                    func.count(distinct(agents.c.project_id)).label("runs"),
                    func.coalesce(func.sum(agents.c.cost_usd), 0).label("cost_usd"),
                    func.count().label("total"),
                    func.count().filter(agents.c.status == "running").label("active"),
                    func.count().filter(agents.c.outcome.in_(("real_diff", "success"))).label("successes"),
                    func.max(agents.c.model).label("model"))
                .group_by(agents.c.role))
        return self._x.fetchall(stmt)

    def agents_active_count(self) -> int:
        row = self._x.fetchone(select(func.count().label("n")).where(agents.c.status == "running"))
        return int(row["n"]) if row else 0

    def today_burn(self, since_epoch: float) -> float:
        row = self._x.fetchone(select(func.coalesce(func.sum(agents.c.cost_usd), 0).label("burn"))
                               .where(agents.c.started_at >= since_epoch))
