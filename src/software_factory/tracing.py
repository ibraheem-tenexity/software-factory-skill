"""Langfuse exporter: project.log events -> LLM traces (trace=run, generation=assistant turn,
event=tool call). Tapped from the console poller — no agent/launch-path changes; works for
both runtimes because it parses the same stream project.log already holds.
...
Event ids are DETERMINISTIC (project_id + log offset), so re-exports after a restart (offsets
...
def _eid(project_id: str, kind: str, seq: int) -> str:
    return hashlib.sha1(f"{project_id}:{kind}:{seq}".encode()).hexdigest()
...
def map_events(project_id: str, text: str, meta: dict | None = None, seq0: int = 0) -> list:
    """project.log lines -> Langfuse ingestion events. seq0 keeps ids unique across ticks."""
    ts = _now()
    out = []
    if seq0 == 0:
        out.append({"id": _eid(project_id, "trace", 0), "type": "trace-create", "timestamp": ts,
                    "body": {"id": project_id, "name": project_id, "metadata": meta or {}}})
    seq = seq0
    for ev in _events(text):
        ...
        if usage:  # claude assistant turn with token usage -> generation
            ...
            out.append({"id": _eid(project_id, "gen", seq), "type": "generation-create",
                        "timestamp": ts, "body": {
                            "id": _eid(project_id, "gen", seq), "traceId": project_id,
        ...
        for c in (msg.get("content") or []) or []:
            if isinstance(c, dict) and c.get("type") == "tool_use":
                out.append({"id": _eid(project_id, "tool", seq), "type": "event-create",
                            "timestamp": ts, "body": {
                                "id": _eid(project_id, "tool", seq), "traceId": project_id,
...
class Tracer:
    ...
    def tick(self, project_id: str, log_path: str, meta: dict | None = None) -> int:
        if not enabled() or not os.path.exists(log_path):
            return 0
        try:
            start = self._offsets.get(project_id, 0)
            ...
            events = map_events(project_id, text, meta=meta, seq0=self._seqs.get(project_id, 0))
            sent = 0
            ...
            self._offsets[project_id] = start + nl + 1
            self._seqs[project_id] = self._seqs.get(project_id, 0) + len(text.splitlines())
            return sent
        except Exception:
            return 0
