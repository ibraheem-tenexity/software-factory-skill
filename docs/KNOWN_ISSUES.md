...
**Resolved (TEN-151, precedence fixed SOF-24).** `GET /api/version` (`console/routers/open_routes.py` → `software_factory.version.version_info`) returns `{sha, short, dirty}`, sourced from `RAILWAY_GIT_COMMIT_SHA` (injected fresh by Railway on every git-source deploy, auto or manual push) → `SF_GIT_SHA` (baked by `scripts/deploy.sh`, the only signal on a manual `railway up` deploy since Railway's git-metadata vars aren't injected there) → `git rev-parse` fallback (dead in prod — no `.git` in the image). RAILWAY_GIT_COMMIT_SHA must come first: it's the only source guaranteed fresh on every deploy, whereas SF_GIT_SHA is a persistent service variable that a native-git-source auto-deploy (SOF-16) never touches — checking it first let a stale value from an old manual deploy silently freeze `/api/version`. Link re-assertion: `software_factory.railway_link.assert_link` parses `railway status` and fails loudly on any project/env/service drift — run it before verifying via `make verify-link` (`scripts/assert-railway-link.sh`).
