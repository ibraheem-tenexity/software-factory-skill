...
2. **Eager draft (form is sole creator)** — `POST /api/drafts` → `{run_id}`. Create ONCE on mount and
   keep the id. Pass it into every write below AND into `POST /api/chat` as `run_id`, so the form and
...
3. **Write-through (debounced / on-blur — NOT per keystroke)**:
   ...
   - Project: `PATCH /api/runs/{run_id}/draft`  body `{name?, goal?, scope?}` (any subset).
   - Materials: `POST /api/runs/{run_id}/attach` body `{files:[{name, content_b64|content, ...}]}`.
4. **Handoff** — `POST /api/runs/{run_id}/promote` body `{target?}` → `{run_id, status:"started"}`
...
|---|---|---|
| `POST /api/drafts` | `{project_name?, runtime?, planning_model?, impl_model?}` | `{run_id}` |
| `PATCH /api/runs/{id}/draft` | `{name?, goal?, scope?}` | `{name, goal, scope, description, brief, coverage}` |
| `POST /api/runs/{id}/attach` | `{files:[…]}` | `{attached:[paths]}` |
| `POST /api/runs/{id}/promote` | `{description?, target?}` | `{run_id, status:"started"}` |
...
| `set_project_basics(name, goal)` / `set_project_scope(scope[])` | `PATCH /api/runs/{id}/draft` |
| `attach_project_materials` (ack) / files on the message | `POST /api/runs/{id}/attach` (auto on chat) |
| `request_materials(what, why)` | (chat-only signal; no endpoint) |
| `get_intake_state` / `validate_intake_complete` | read-only |
| `hand_off_to_factory(target)` | `POST /api/runs/{id}/promote` |
...
draft, but it doesn't push form-state changes. Refetch the draft (`GET /api/runs/{id}/brief` +,
