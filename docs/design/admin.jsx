    ...
    <React.Fragment>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, height: 36, padding: '0 11px', borderRadius: T.rMd, border: `1px solid ${T.borderDefault}`, background: T.raised, flex: 1, minWidth: 200 }}>
          <Icon name="search" size={14} color={T.tertiary} /><span style={{ font: `400 12.5px/1 ${T.sans}`, color: T.tertiary }}>Search name, organization, factory…</span>
        </div>
        <AdminFilter>All organizations</AdminFilter><AdminFilter>All factories</AdminFilter><AdminFilter w={130}>All statuses</AdminFilter><AdminFilter w={130}>All modes</AdminFilter>
      </div>
      ...
      <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: T.rLg, overflow: 'hidden', background: T.raised }}>
        ...
        {ADMIN_PROJECTS.map((p, i) => (
          <button key={p.name + i} style={{ width: '100%', textAlign: 'left', cursor: 'pointer', background: T.raised, border: 'none', borderTop: i ? `1px solid ${T.borderSubtle}` : 'none',
            ...
            <span style={{ font: `400 12.5px/1.3 ${T.sans}`, color: T.secondary, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.client}</span>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '16px 20px' }}>
          {tab === 'prompt' && (
            <React.Fragment>
              ...
              <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', minHeight: 280, padding: '13px 15px', borderRadius: T.rMd, resize: 'vertical', border: `1px solid ${T.borderDefault}`, background: T.bg, color: T.fg, font: `400 13px/1.65 ${T.mono}`, outline: 'none' }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 14 }}>
        <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: T.rLg, background: T.raised, overflow: 'hidden' }}>
          ...
          {ADMIN_PROJECTS.slice(0, 6).map((p, i) => (
            <div key={p.name + i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px', borderTop: i ? `1px solid ${T.borderSubtle}` : 'none' }}>
