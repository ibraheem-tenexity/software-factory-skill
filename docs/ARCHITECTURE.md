...
|---|---|
| `console.py` | The temporary execution facade: `_provision_and_launch`, stage gates (`detect_stage{1,2,3}_done`), budget, `list_projects`/`status`/`graph`/`tickets`/`deployments` projections, and ownership remain here. It composes the `projects` intake owner while execution callers migrate. `ProjectRequest` is the stage-launch contract. |
| `projects/intake.py` | **Projects bounded context:** owns the durable onboarding record: draft creation and updates, canonical project-description composition, draft attachments, product-brief reads, BYOK credential vault references, repository-access request/status, and the shared project path projection. Production callers use `Console.intake` until application composition moves out of `Console`. |
...
| `services/` | Framework-free application logic between routers and data access: `OrgService`, `Secrets`, `DbConversation`, and `AdminService` are wired in `console/state.py::reset()`. `DbConversation` owns durable onboarding history and creates one intake `ChatAgent` per project; `console/chat_dock.py` separately creates project/context agents for the persistent dock. They share the same identity, DB prompt, agent class, and tools—not the same Python object. `services/errors.py` maps domain errors to HTTP responses; repositories hold parameterized SQLAlchemy Core CRUD. |
