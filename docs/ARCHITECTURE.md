...
|---|---|
| `console.py` | The temporary compatibility facade and execution orchestrator: `Console` still exposes the established intake methods, while `_provision_and_launch`, stage gates (`detect_stage{1,2,3}_done`), budget, `list_projects`/`status`/`graph`/`tickets`/`deployments` projections, and ownership remain here. `ProjectRequest` is the stage-launch contract. |
| `projects/intake.py` | **Projects bounded context:** owns the durable onboarding record: draft creation and updates, canonical project-description composition, draft attachments, product-brief reads, BYOK credential vault references, repository-access request/status, and the shared project path projection. `Console` forwards its legacy intake surface here while active callers migrate. |
