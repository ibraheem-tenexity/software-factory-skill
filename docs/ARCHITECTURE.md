...
| `chat_agent.py` | The "Factory Concierge" — an OpenAI-Agents-SDK agent that interviews the user over the draft minted at onboarding start, then calls its `hand_off_to_factory` tool (== `promote_draft`) once a product brief is finalized (and answers status/deps questions). Its effective prompt is `CONCIERGE_INSTRUCTIONS` plus an env-safe, 60s TTL cached `agent_prompts.CONCIERGE` override so prompt edits apply to new concierge sessions without per-turn DB latency. |
...
- **Create a project (Option C, the only path):** `POST /api/drafts` mints the canonical id
  (`create_draft`, phase="draft", no artifacts recorded yet — the poller ignores it) → the
