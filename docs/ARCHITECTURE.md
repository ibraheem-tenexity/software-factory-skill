...
│   │  console/app.py   FastAPI/uvicorn (ASGI) + workers/supervisor.py every 3s             │  │
...
│   │     • supervisor: auto-advance stages, enforce budget, narrate, export traces         │  │
│   │  execution.service.ExecutionService   the orchestrator (promote_draft, stage launches,│  │
...
|---|---|
| `console.py` | Compatibility exports for the retired monolithic import path. New production composition uses `execution/service.py`; keep this shim until active operator/external callers move. |
| `execution/prompts.py`, `execution/process.py`, `execution/service.py` | **Execution bounded context:** `prompts.py` owns the `ProjectRequest` and stage-specific variable context; `process.py` owns OS process inspection/launch; `service.py` owns lifecycle, recovery, budgets, provision, promotion, gates, dependency handoff, and run status. Its remaining project views/mutations and cleanup/reaper policy are explicitly transitional, with the next moves owned by `projects/` and `cleanup/`. |
| `workers/supervisor.py` | Factory supervision worker: boot sweep, auto-advance, recovery/reaper and log-flush ticks, health, and application lifespan. The console app only wires its lifespan. |
| `conversation/dock.py`, `conversation/persistence.py` | **Conversation bounded context:** the in-project chat runtime and durable chat/tool-trace persistence. The old `console/chat_*` modules are compatibility exports only. |
| `projects/intake.py` | **Projects bounded context:** owns the durable onboarding record: draft creation and updates, canonical project-description composition, draft attachments, product-brief reads, BYOK credential vault references, repository-access request/status, and the shared project path projection. Production callers use `ExecutionService.intake` until application composition moves out of the execution service. |
...
| `chat_agent.py`, `conversation/concierge_prompt.py`, `concierge_tools.py` | The Factory Concierge is a LangChain `create_agent` runtime with a real per-project tool belt. The base instructions come only from `system_agents(callsign='CONCIERGE').prompt`; there is no code default. `conversation/concierge_prompt.py` short-caches the last known good DB prompt for 60 seconds, then appends code-owned context framing and first-turn project context to form the effective system prompt. A missing/blank row fails agent construction; a DB read error may use only a previously loaded DB value. The agent may call `hand_off_to_factory` after agreement; completed onboarding turns report factual `handed_off` state so the UI navigates on tool success. |
...
| `skills/stage-{1,2,3}-*` | The stage contracts (Claude and OpenCode variants); `skills/tenexity-design/` is the vendored brand canon. Tenexity OS surfaces the three stage skills and the Concierge as live orchestrator cards. Stage prompt edits are stored in one `system_agents` row per bare callsign, so one edit applies to both runtime variants on the next launch. The Concierge's DB prompt is read through its 60-second cache. Specialist cards remain stored but not applied to runtime prompts. |
| `recipes/store.py` | **Recipes bounded context (CBT-9, SOF-202):** the `recipes` table CRUD + the one fact gate — saving a `repo_url` shallow-clones and requires `AGENTS.md`/`CLAUDE.md` at the repo root, refusing with the verbatim reason (`RecipeValidationError` → HTTP 400). `published()` is the customer picker source (light fields, public images only). A selected recipe's `body_md` becomes the Concierge/brief framing and lands as `input/recipe.md` at promote; its repo is cloned into the stage-3 workspace as the build seed with a fork-and-extend SKILL block. No selected recipe means intake proceeds from the user's own context; the retired SOW system is not a fallback. Admin CRUD lives in `admin_os`; `GET /api/recipes` serves intake. |
...
- **Project Memory (SOF-26):** `doc_summary` (PK `blob_id`→`blobs.id` cascade) — the per-document
  ...
  yet). Both are `scope`/`scope_id`-filtered like `blobs`, so
...
- **Conversation store (SOF-26):** `conversation` — one row per message/turn (PK `id` = the
  ...
  `model`/`provider`/token/`cost_usd` attribution. It replaced the in-memory `/converse` mock and
  volume-only `chat.jsonl`; both onboarding and dock conversations now persist here.
- `system_agents` — one editable prompt/model row per orchestrator callsign. Migration 0032 seeds
  `CONCIERGE` only when absent/blank; subsequent non-empty operator edits are authoritative. Runtime
  code never seeds or fabricates a fallback prompt. `tools` — the stage-attached
...
- `recipes` (migration 0029) — repo-backed build recipes: customer-facing fields (name/tagline/
  ...
  draft to one; no recipe means the project uses its own intake/brief without template fallback.
...
- **Multi-deliverable:** a project ships **1..N deliverables**; per-app deploy/verify state lives in the
  ...
  (→ `ExecutionService.records.deployments`). Source-of-truth ERD: [`schema-erd.svg`](schema-erd.svg).
...
and `ExecutionService` snapshots active `project.log` content, and user-document Markdown is persisted as artifacts.
