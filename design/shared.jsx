...
const PATHS = {
  ...
  restore: 'M3 2v6h6 M3.51 9a9 9 0 1 0 2.13-3.36L3 8',
  ...
  palette: 'M12 22a10 10 0 1 1 10-10c0 1.66-1.34 3-3 3h-2.5a2.5 2.5 0 0 0-1.8 4.2c.4.43.6.99.6 1.55A1.25 1.25 0 0 1 12 22z M7.5 10.5a1 1 0 1 0 0-2 1 1 0 0 0 0 2z M12 7.5a1 1 0 1 0 0-2 1 1 0 0 0 0 2z M16.5 10.5a1 1 0 1 0 0-2 1 1 0 0 0 0 2z',
...
// Shared across every text field. Appends speech to the field's current value.
// Gracefully no-ops (renders no mic) on browsers without SpeechRecognition.
function useDictation(value, onChange) {
  ...
  const toggle = () => {
    ...
    baseRef.current = (valRef.current || '').trim();
    rec.onresult = (e) => {
      let txt = '';
      for (let i = 0; i < e.results.length; i++) txt += e.results[i][0].transcript;
      onChange && onChange((baseRef.current ? baseRef.current + ' ' : '') + txt.replace(/^\s+/, ''));
    };
...
function MicButton({ listening, onClick, size = 28, title = 'Dictate' }) {
  return (
    <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={onClick} title={listening ? 'Stop dictation' : title} aria-label={title}
      style={{ width: size, height: size, flexShrink: 0, display: 'grid', placeItems: 'center', borderRadius: T.rMd, border: 'none', cursor: 'pointer',
        background: listening ? T.danger : 'transparent', color: listening ? '#fff' : T.tertiary, transition: 'all .15s' }}>
      <Icon name="mic" size={Math.round(size * 0.52)} color={listening ? '#fff' : T.tertiary} />
    </button>
  );
}

function TextInput({ value, onChange, placeholder, type = 'text', style, mono, onKeyDown, size = 'md', noMic }) {
  const h = size === 'sm' ? 32 : 36;
  const dict = useDictation(value, onChange);
  const showMic = !noMic && dict.supported && type !== 'password';
  const input = (
    <input type={type} value={value || ''} onChange={(e) => onChange && onChange(e.target.value)} placeholder={placeholder} onKeyDown={onKeyDown}
      style={{ width: '100%', boxSizing: 'border-box', height: h, padding: '0 10px', borderRadius: T.rMd,
        ...
        font: `400 13px/1 ${mono ? T.mono : T.sans}`, outline: 'none', ...style, ...(showMic ? { paddingRight: 32 } : null) }} />
  );
  if (!showMic) return input;
  return (
    <div style={{ position: 'relative', width: '100%', display: 'block', ...style, padding: 0, border: 'none', background: 'transparent', height: 'auto', boxShadow: 'none', borderRadius: 0 }}>
      {input}
      <span style={{ position: 'absolute', right: 3, top: 0, bottom: 0, display: 'flex', alignItems: 'center' }}>
        <MicButton size={h - 8} listening={dict.listening} onClick={dict.toggle} />
      </span>
    </div>
  );
}

function TextArea({ value, onChange, placeholder, rows = 4, style, onKeyDown, noMic }) {
  const dict = useDictation(value, onChange);
  const showMic = !noMic && dict.supported;
  const ta = (
  ...
  return (
    <div style={{ position: 'relative', width: '100%', display: 'block', ...style, padding: 0, border: 'none', background: 'transparent', height: 'auto', boxShadow: 'none', borderRadius: 0, resize: 'none' }}>
      {ta}
      <span style={{ position: 'absolute', right: 6, top: 7, display: 'flex' }}>
        <MicButton size={28} listening={dict.listening} onClick={dict.toggle} />
      </span>
    </div>
  );
}
...
function FileDescRow({ file, last, describe, scopable }) {
  ...
  const descDict = useDictation(desc, setDesc);
  const autofill = () => {
    setBusy(true);
      ...
      {describe && (
        <div style={{ marginTop: 9, marginLeft: 42, position: 'relative' }}>
          <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={2}
            ...
            style={{ width: '100%', boxSizing: 'border-box', padding: '8px 40px 26px 10px', borderRadius: T.rMd, resize: 'none',
          ...
          {descDict.supported && <span style={{ position: 'absolute', right: 6, top: 6 }}><MicButton size={26} listening={descDict.listening} onClick={descDict.toggle} /></span>}
...
function Composer({ placeholder = 'Reply…', onSend, value, onChange }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, border: `1px solid ${T.borderDefault}`, borderRadius: T.rLg, background: T.raised, padding: 8 }}>
      ...
      {dict.supported && <MicButton size={30} listening={dict.listening} onClick={dict.toggle} />}
...
// ---------- inline markdown ----------
// Lightweight renderer for short free-text fields (e.g. the project goal the
// customer typed). Renders **bold**, *italic*, `code`, [links](url), and
// "- " / "1." bullet lists. If no markdown tokens are present it renders the
// text verbatim, so plain prose is unaffected.
const MD_INLINE_RE = /(\*\*[^*]+\*\*|\*[^*]+\*|_[^_]+_|`[^`]+`|\[[^\]]+\]\([^)]+\))/g;
function looksLikeMarkdown(s) {
  return typeof s === 'string' && /(\*\*[^*]+\*\*|\*[^*]+\*|_[^_]+_|`[^`]+`|\[[^\]]+\]\([^)]+\)|^\s*[-*]\s+|^\s*\d+[.)]\s+)/m.test(s);
}
function renderInlineMd(text, key) {
  const out = [];
  let i = 0;
  text.split(MD_INLINE_RE).forEach((tok, n) => {
    ...
    else if (/^\[[^\]]+\]\([^)]+\)$/.test(tok)) { const m = tok.match(/^\[([^\]]+)\]\(([^)]+)\)$/); out.push(<a key={k} href={m[2]} style={{ color: T.brandDeep, textDecoration: 'underline' }}>{m[1]}</a>); }
    else out.push(<React.Fragment key={k}>{tok}</React.Fragment>);
    i++;
  });
  return out;
}
// Renders markdown when detected; otherwise plain text. `style` styles the wrapper.
function GoalMarkdown({ children, style }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7, ...style }}>
      {blocks.map((b, i) => b.items
        ...
        : <p key={i} style={{ margin: 0 }}>{renderInlineMd(b.p, `${i}`)}</p>)}
    </div>
  );
}
...
Object.assign(window, {
  ...
  GoalMarkdown, looksLikeMarkdown, useDictation, MicButton,
});
