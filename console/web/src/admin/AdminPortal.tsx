...
import { Icon, Sparkle, StatusPill, Field, TextInput, MetricCard } from "./primitives";
...
type Project = {
  ...
  client: string;
  factory: string;
  phase: "REVIEW" | "PLANNING" | "BUILDING" | "TRIAGE" | "INTAKE" | "LIVE";
  tasks: string;
  f: string;
  auto: number;
  last: string;
...
const PHASE_TONE: Record<Project["phase"], "info" | "warning" | "brand" | "neutral" | "success"> = {
  REVIEW: "info",
  PLANNING: "warning",
  BUILDING: "brand",
  TRIAGE: "neutral",
...
const ROSTER: Agent[] = [
  {
    name: "Orchestrator",
    callsign: "ORCHESTRATOR.MAIN",
    sign: "ATLAS",
    desc: "Main orchestrator across all factories",
    model: "claude-opus-4",
    cost: 3,
    success: 97,
    on: true,
  ...
  },
  {
    ...
    callsign: "PM.LEAD",
    sign: "HORIZON",
    desc: "Translates conversation into specs",
  ...
  {
    name: "Design Lead",
    callsign: "DESIGN.LEAD",
    sign: "CHROMA",
    desc: "Brand, layout, and design system",
  ...
  {
    name: "Marketing Lead",
    callsign: "MARKETING.LEAD",
    sign: "SIREN",
    desc: "Positioning, ads, content",
    model: "gpt-5-thinking",
    cost: 2,
    success: 90,
    on: true,
  ...
  {
    name: "Proposal Lead",
    callsign: "PROPOSAL.LEAD",
    sign: "TENDER",
    desc: "Pricing, contracts, scope",
    model: "claude-sonnet-4",
    cost: 2,
    success: 93,
    on: true,
  ...
  {
    name: "DevOps Lead",
    callsign: "DEVOPS.LEAD",
    sign: "FORGE",
    desc: "Infra, CI, secrets, deploys",
    model: "claude-sonnet-4",
    cost: 2,
    success: 95,
    on: true,
  ...
  },
  {
    ...
    callsign: "OPS.LEAD",
    sign: "GARRISON",
    desc: "Internal tools and runbooks",
    model: "claude-sonnet-4",
    cost: 2,
    success: 93,
    on: false,
  ...
  },
  {
    ...
    callsign: "DATA.LEAD",
    sign: "MATRIX",
    desc: "Pipelines, warehouses, schemas",
    model: "claude-sonnet-4",
    cost: 2,
    success: 91,
    on: false,
  ...
  },
  {
    ...
    callsign: "EDI.LEAD",
    sign: "LEDGER",
    desc: "Trading-partner mappings: 850, 855, 856, 810, 997",
    model: "claude-sonnet-4",
    cost: 2,
    success: 95,
    on: true,
  ...
  {
    name: "ERP Integration Lead",
    callsign: "ERP.LEAD",
    sign: "CONDUIT",
    desc: "SAP, NetSuite, Oracle, Epicor bridges",
    model: "claude-sonnet-4",
    cost: 2,
    success: 90,
    on: true,
  ...
  },
  {
    ...
    callsign: "WMS.LEAD",
    sign: "CARGO",
    desc: "Manhattan, Blue Yonder, High Jump, Körber bridges",
    model: "claude-sonnet-4",
    cost: 2,
    success: 89,
    on: false,
  ...
  {
    name: "Pricing & Margin Lead",
    callsign: "PRICING.LEAD",
    sign: "PROFIT",
    desc: "Tiered pricing, contract pricing, margin leakage",
    model: "claude-sonnet-4",
    cost: 2,
    success: 91,
    on: false,
...
function PhasePill({ phase }: { phase: Project["phase"] }) {
  ...
  return (
    <span style={{ display: "inline-flex", gap: 2 }}>
      {[0, 1, 2].map((i) => (
        <span
          key={i}
          style={{ width: 5, height: 5, borderRadius: "50%", background: i < n ? T.fg : T.borderDefault }}
        />
      ))}
    </span>
  );
  ...
  return (
    <>
      <PageTitle
        ...
        actions={
          <>
            <AdminBtn>⟳ Import from Asana</AdminBtn>
            <AdminBtn primary>+ New client</AdminBtn>
          </>
        }
      />
      <div
      ...
      <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: T.rLg, overflow: "hidden", background: T.raised }}>
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "minmax(0,1fr) 130px 140px 130px 130px",
            gap: 16,
            padding: "11px 18px",
...
function AgentCard({ a, onOpen }: { a: Agent; onOpen: (a: Agent) => void }) {
  return (
    <button
      onClick={() => onOpen(a)}
      style={{
  ...
  return (
    <>
      <PageTitle
        ...
        actions={<AdminBtn>⌥ Configure repo</AdminBtn>}
      />
      <div
        style={{
          display: "flex",
...
function AgentPromptPanel({ agent, onClose }: { agent: Agent; onClose: () => void }) {
      ...
      >
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "16px 20px" }}>
          {tab === "prompt" && (
            <>
              ...
              <Mono style={{ fontSize: 10.5, marginTop: 8, display: "block" }}>
                {prompt.length} chars · version 0.1.0 · last edited by ATLAS
              </Mono>
            </>
          )}
          {tab === "tools" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              {MCP_TOOLS.filter((t) => t.status === "connected")
                .slice(0, 5)
                .map((t) => (
                  <div
                    key={t.name}
          ...
          {tab === "activity" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {[
                ["Claimed SF-05 · Discount approval workflow", "4m ago"],
                ["Pushed commit a3f91 to acme-quote-to-erp", "11m ago"],
                ["Opened PR #42 · pricing rules", "18m ago"],
                ["Prompt edited · +1 guardrail", "2h ago"],
              ].map((r, i) => (
                <div
                  key={i}
                  style={{
        ...
        >
          <span style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: agent.on ? T.success : T.borderDefault }} />
            <Mono style={{ fontSize: 11.5 }}>{agent.on ? "active" : "idle"}</Mono>
          </span>
          <div style={{ display: "flex", gap: 9 }}>
            <AdminBtn onClick={onClose}>Cancel</AdminBtn>
            <AdminBtn primary onClick={onClose}>
  ...
  return (
    <>
      <PageTitle
        ...
        actions={
          <>
            <AdminBtn>⟳ Sync registry</AdminBtn>
            <AdminBtn primary>+ Register tool</AdminBtn>
          </>
        }
      />
      ...
      <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: T.rLg, overflow: "hidden", background: T.raised }}>
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "minmax(0,1fr) 80px minmax(0,0.9fr) minmax(0,1fr) 120px 70px 90px",
            gap: 12,
            padding: "11px 18px",
            borderBottom: `1px solid ${T.borderSubtle}`,
            background: T.sunken,
  ...
  return (
    <>
      <PageTitle
        ...
        actions={<AdminBtn primary onClick={() => onNav("projects")}>View all projects</AdminBtn>}
      />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 18 }}>
        <MetricCard label="Tenants" value={ADMIN_CLIENTS.length} hint="active organizations" accent />
        ...
        <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: T.rLg, background: T.raised, overflow: "hidden" }}>
          ...
          {ADMIN_PROJECTS.slice(0, 6).map((p, i) => (
            ...
            >
              ...
              <PhasePill phase={p.phase} />
              <Mono style={{ fontSize: 10.5, width: 80, textAlign: "right" }}>{p.last}</Mono>
            </div>
          ))}
        </div>
        <div style={{ border: `1px solid ${T.borderSubtle}`, borderRadius: T.rLg, background: T.raised, overflow: "hidden" }}>
...
function InviteModal({ onClose }: { onClose: () => void }) {
  ...
  const [type, setType] = React.useState<"New org" | "Tenexity">("New org");
  const [org, setOrg] = React.useState("");
  const [list, setList] = React.useState<AllowedUser[]>(ALLOWED);
    ...
    setList((l) => [
      {
        email,
        type,
        org: type === "Tenexity" ? "Tenexity" : org || "New organization",
        role: type === "Tenexity" ? "Operator" : "Org admin",
        status: "invited",
      },
      ...l,
    ]);
        ...
        <div style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
            ...
            <Field label="Access type" style={{ width: 150, flexShrink: 0 }}>
              ...
              >
                ...
                >
                  <option>New org</option>
                  <option>Tenexity</option>
                </select>
          ...
          >
            <Mono style={{ fontSize: 11 }}>
              {type === "New org" ? "Becomes ORG ADMIN · email added to allow-list" : "Added to allow-list as OPERATOR"}
            </Mono>
            <AdminBtn primary onClick={send}>
              Send invite
            </AdminBtn>
          </div>
        </div>
        <div style={{ borderTop: `1px solid ${T.borderSubtle}`, overflow: "auto" }}>
          ...
          {list.map((u, i) => (
            <div
              ...
              style={{
                display: "grid",
                gridTemplateColumns: "minmax(0,1fr) 130px 90px 80px",
                gap: 10,
                alignItems: "center",
                padding: "11px 20px",
                borderTop: i ? `1px solid ${T.borderSubtle}` : "none",
      ...
      <div style={{ height: "100%", position: "relative", display: "flex", background: T.bg, fontFamily: T.sans }}>
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              ...
              <Mono style={{ fontSize: 10.5, color: T.success }}>LINEAR · PHI</Mono>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 6 }}>
              <Mono style={{ fontSize: 10 }}>Sys Status</Mono>
              <Mono style={{ fontSize: 10, color: T.success }}>Nominal</Mono>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
          ...
          >
            ...
            <AdminBtn primary onClick={() => setInvite(true)}>
            ...
            {(view === "factories" || view === "settings") && (
              <div style={{ display: "grid", placeItems: "center", height: 320 }}>
                <div style={{ textAlign: "center" }}>
                  <Mono style={{ fontSize: 12, display: "block", marginBottom: 6 }}>{view.toUpperCase()}</Mono>
                  <span style={{ font: `400 14px/1.5 ${T.sans}`, color: T.tertiary }}>
