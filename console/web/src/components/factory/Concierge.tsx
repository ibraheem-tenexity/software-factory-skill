  ...
  const steer = async (text?: string) => {
    ...
    try {
      const resp = await api.chatStream(
        // SOF-246: display_context is the ephemeral per-peer grounding (#446's ChatIn.display_context)
        // — injected into THIS turn's prompt only, never persisted, never a retrieval boundary.
        { project_id: projectId, project_name: projectName || "", message: msg,
          display_context: displayContextStr(context, selectedLabel) },
        ctrl.signal,
      );
