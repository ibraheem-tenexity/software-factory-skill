...
export function FactoryActivity({ projectId, artifacts, onOpenArtifact, selectedEventId, designPanel, depsPanel }: {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {/* Stage-triggered action panels render at the chronological tail (the current live gates),
          INSIDE Activity only — selecting Kanban/Tree/Map removes them from layout entirely, so they
          can never push another selected view below the viewport. Design review (the design gate,
          Stage 2) precedes wait-for-deps (the Stage 2→3 boundary); each self-gates and shows only
          when it is the live gate. */}
      {designPanel}
      {depsPanel}
    </div>
  );
