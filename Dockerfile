...
RUN pip3 install --break-system-packages openai-agents 'markitdown[pdf,docx]' pypandoc_binary mammoth markdownify 'psycopg[binary]>=3.1' fastapi 'uvicorn[standard]' 'alembic>=1.13' 'google-auth>=2.28' 'python-dotenv>=1.0' 'langfuse>=4.12.0' 'openinference-instrumentation-openai-agents>=1.6.1' 'pgvector>=0.3.6'
