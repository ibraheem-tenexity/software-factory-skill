...
# SOF-228 markitdown .env leak in dev worktrees (2026-07-22)
1. `import markitdown` triggers python-dotenv autoload; `find_dotenv` walks UP from CWD and loads the nearest `.env`. In a dev worktree that is the repo's LIVE `/home/ibraheem/software-factory-skill/.env`, so the import silently injected SF_GOOGLE_CLIENT_ID / SF_SESSION_SECRET (bisect-proven, reproduced: both keys False→True) into os.environ — flipping a local auth-off console to authed the moment the first PDF/doc is parsed (markitdown is imported lazily in `pdf_extract._markitdown_convert`). Deployed containers are unaffected (no parent `.env`).
2. Fix (`pdf_extract._import_markitdown`): snapshot os.environ, import markitdown, then restore — drop keys the autoload ADDED, revert any it CHANGED. Provider-agnostic (protects every secret, not just the two auth keys). Verified: guarded import leaks nothing and `MarkItDown().convert()` still works.
3. Immunization for any OTHER dev process that imports markitdown directly: pre-set `SF_GOOGLE_CLIENT_ID=""` (python-dotenv `load_dotenv` uses override=False, so it respects an already-set value and won't overwrite). The code fix above already covers the factory's own ingestion path.
# UI review rule (SOF-236, 2026-07-22)
1. Every UI change requires Playwright review via the available MCP/plugin; Codex uses its in-app browser Playwright interface when available.
# SOF-251 source-directory schema + doc migration (2026-07-22, epic SOF-238 Files)
1. New `directories` relation (uuid PK; scope/scope_id mirror blobs; nullable parent_id => per-scope root; summary_md/summary_status[summarizing|ready|needs_refresh|failed]/summary_source_hash/last_successful_summary_at) + nullable `blobs.directory_id`. Composite FKs pin (id,scope,scope_id): parent-same-scope + blob-same-scope; both ON DELETE RESTRICT (no silent cascade/orphan). Two partial unique indexes for sibling names (parent vs NULL root). Table def in models.py (single source); ERD + ARCHITECTURE updated.
2. Migration `0034_source_directories` (down_revision 0033_artifact_stage — HELD, chains off #441/SOF-78). Files each top-level source blob (source_blob_id IS NULL) under one per-scope root (name=project/org display name, else scope_id); extracted children stay provenance-linked, directory_id NULL. Only writes directory_id — no blob id/key/hash/summary/chunk/use/scope change. Idempotent (IF NOT EXISTS / ON CONFLICT / directory_id IS NULL). Roots init summary_status='needs_refresh' (no rollup ever computed = honest).
3. GOTCHA: blobs' composite directory FK breaks `set_scope` on a scope move (dir belongs to old scope) — repositories/blobs.set_scope now clears directory_id on move (blob lands unfiled in new scope; SOF-253 re-homes). SOF-253 read projection: per-scope tree of directories (id,parent_id,name,summary_status,...) + membership counts, blobs joined via directory_id; virtual mixed-scope top-level root is synthesized, never a row.
# SOF-253 source-directory APIs + directory-aware file mutations (2026-07-22, epic SOF-238 Files, stacks on SOF-251)
1. Read projection `project_view.files_tree(scopes)`: synthesized virtual combined root (id:null, is_virtual, NEVER a DB row) + persisted per-scope `roots` + full `directories` (parent/scope/child_dir_count/member_file_count/summary state/timestamps) + `files` (stable blob id, directory_id, scope, kind, size, sha, ingest+summary status, summary) + `recent` (references into files, not duplicate membership). Only top-level source blobs (source_blob_id IS NULL) are tree members. `ProjectMaterials.files(pid)` gathers project scope + owner-org scope ONLY (never leaks another tenant); ensures each scope's root exists lazily.
2. Directory SQL lives in `repositories/blobs.BlobRepository` + facade in `blobs.BlobStore` (one source-material persistence boundary, no new pass-through layer): list_directories/directory_by_id/find_root/insert_directory/sibling_name_exists/set_directory/touch_directory; `insert` + `record` gained `directory_id` (membership written in the SAME blob insert = no successful orphan). `_BLOB_COLS` now also returns directory_id (uuid→str) + created_at (epoch).
3. Mutations own policy in `ProjectMaterials` (raise services/errors ServiceError → mapped by app.py; routers thin): create_directory (real scoped parent, precise 409 on dup sibling), upload (project scope, defaults to project root), move_file (within-scope re-home), set_scope (cross-scope, existing scope policy + re-home under dest dir or dest root; validates dest BEFORE the scope change so no moved-but-unfiled), delete (unchanged cleanup + touch source dir). All authorize scope via `_authorize_scope`: project=this project, org=owner org (matches existing document access, does NOT weaken); virtual root + extracted children rejected with honest reasons. set_scope gained a scope guard closing a pre-existing cross-tenant re-scope gap.
4. New routes (all Depends(authorize_project)): GET /api/projects/{pid}/files; POST /api/projects/{pid}/directories {parent_id,name}; POST /api/projects/{pid}/files {name,data_b64,tag?,content_type?,directory_id?}; PATCH /api/projects/{pid}/files/{blob_id} {directory_id?,scope?} (scope set => cross-scope move, else within-scope); DELETE /api/projects/{pid}/files/{blob_id}. Existing /materials + /documents routes keep their documents() contract (mutations now return identity; routes pick the projection). Frontend api.ts NOT touched — backend-only ticket; SOF-255 builds the Files browser to this contract.
# SOF-254 read-only directory summaries + ancestor invalidation (2026-07-22, epic SOF-238 Files, stacks on SOF-253)
1. `memory/directories.py` (new) is the ONLY writer of `directories.summary_*` (read-only by construction; no edit path). `refresh_directory` rolls a folder up from DIRECT child-file `doc_summary` text + DIRECT child-directory summaries, then appends a code-computed incomplete-coverage note (failed/pending child never silently dropped). Empty dir => ready + NULL summary (no fabricated prose). Unchanged (ready + `summary_source_hash`==current subtree hash) => skipped, no model call. Reuses ingest.py chat client/model + cost attribution (project scope).
2. ONE invalidation path `directories.on_directories_changed(dir_ids, console)`: marks each dir's ancestor chain needs_refresh (only flips ready/summarizing; leaves failed to keep its detail) then fires ONE bottom-up `refresh_scope` per affected scope. REPLACED SOF-253's `touch_directory` at all 5 materials.py sites (upload=dest, delete=member dir, move_file=old+new, set_scope=old+dest) so there aren't two half-overlapping mechanisms. Also wired at all 4 ingest_blob terminals (done + parse/embed/summarize fail) via directory_id. NULL dir id = no-op (nothing rolls up from an unfiled blob).
3. New col `directories.summary_error` (migration `0035_directory_summary_error`, down_revision 0034). failed retains last summary_md/hash/last_successful_summary_at + real error; ready clears it. ready only written with CURRENT hash so a failed refresh stays non-ready and retries; blank model reply => failed, never ready-empty. Added `directories` to models GLOBAL_TABLES (SOF-251 omitted it). Rebased onto #448 (SOF-253); merge slot after #448.
# SOF-229 OpenRouter dynamic-prompt-ceiling + opencode QA context bloat (2026-07-23)
1. OPS GOTCHA — OpenRouter's per-key max-PROMPT allowance SHRINKS as the key's remaining DAILY headroom drops. Live: opencode/K3 run `project-07ca737c6f8141fd` died `402 isRetryable:false "Prompt tokens limit exceeded: 227466 > 205309"` with ~$7.70 left of a $70/day key (ceiling ~205k). SMALL calls kept succeeding — so a health probe at small prompt size MISLEADS; you must probe at (or near) the FAILING prompt size to see the ceiling. Distinct from SOF-211's flat zero-credits (that's out of money; this is a moving prompt cap while money remains). Mitigations: raise the key's daily limit, wait for the daily reset, or shrink the prompt (see #2).
2. ROOT CAUSE (product) — stage-3 QA runs in the SAME monolithic opencode session as build+deploy+review (opencode has NO Task/subagent isolation — see workspace_setup.py), so every Playwright snapshot ACCUMULATES (~323k cache-read by the end) and the next prompt crossed the ceiling → crash-park at `test` with the app already built+deployed. FIX (this PR, prompt-level, SKILL.opencode.md Phase 3): a "Playwright context economy" discipline — the datastore (record-verification + storage-persisted screenshots + bug reports) is the source of truth, so extract each flow's finding and move on, prefer targeted reads over full-page snapshots, don't re-snapshot unchanged views, continue each ticket's QA from recorded state. Claude runtime is unaffected (its QA runs in isolated Task subagents).
3. DEEPER FOLLOW-UP (not this PR) — the robust fix is a FRESH opencode session for QA per deliverable/round (QA needs only the deployed URL + demo creds + ticket list, not the build history). That's a driver change in the opencode stage launch/segmentation (swarm_stage3.py / execution/service.py), needs a live opencode multi-deliverable run to verify the <100k-prompt AC, and should be scoped separately. Interplay: SOF-215 (opencode cost accounting) — bigger contexts also inflate the cost meter.

# SOF-240 Concierge Product Brief contract (2026-07-22)
1. Migration `0036_concierge_product_brief_contract` (down_revision 0035_directory_summary_error) adds the flexible, judgment-based brief contract to the DB-backed Concierge prompt only when it still exactly matches the 0032 seed; operator-edited prompts are preserved and surfaced for manual reconciliation.
2. The prompt requires customer read-back and revision through `finalize_product_brief`; artifact existence remains the only mechanical handoff gate.
