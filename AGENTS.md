...
The integrator MERGES each PR through the PR itself (`gh pr merge` — merge/squash/rebase) so GitHub auto-closes it and the PR↔main mapping stays accurate. It does NOT cherry-pick or push commits straight into main — that leaves stale "open" PRs and a misleading list. If a PR needs rebasing first, rebase the branch + update the PR, then merge the PR; never bypass the PR merge. Batching is fine: merge each PR (each auto-closes), then one `railway up`. 