"""Ephemeral per-project workspace: create -> build inside it -> publish -> destroy.
...
def test_create_makes_the_dir_with_a_sentinel(tmp_path):
    ws = workspace.create(str(tmp_path), "project-1")
    assert os.path.isdir(ws)
    assert ws.endswith(os.path.join("project-1", "workspace"))
...
def test_destroy_refuses_a_dir_without_the_sentinel(tmp_path):
    ...
    rogue = tmp_path / "project-x" / "workspace"
    rogue.mkdir(parents=True)
    (rogue / "precious.txt").write_text("do not delete")
    with pytest.raises(Exception):
        workspace.destroy(str(rogue), projects_dir=str(tmp_path))
    assert rogue.exists()
...
def test_destroy_refuses_a_path_outside_runs_dir(tmp_path):
    ...
    other_runs.mkdir()
    with pytest.raises(Exception):
        workspace.destroy(ws, projects_dir=str(other_runs))
    assert os.path.exists(ws)  # untouched


def test_proof_artifacts_at_the_base_survive_teardown(tmp_path):
    # agents.db / tickets.db / projectstate live at the BASE, beside the workspace, not inside it.
    base = tmp_path / "project-1"
    ws = workspace.create(str(tmp_path), "project-1")
