...
def test_noop_when_unconfigured(tmp_path, monkeypatch):
    ...
    p = tmp_path / "project.log"
    p.write_text(CLAUDE_LINE + "\n")
    assert tracing.Tracer().tick("project-x", str(p)) == 0
...
def test_maps_both_runtimes_and_ships_incrementally(tmp_path, sink):
    ...
    p.write_text(CLAUDE_LINE + "\n")
    tr = tracing.Tracer()
    n1 = tr.tick("project-x", str(p), meta={"runtime": "claude"})
    flat = [e for b in sink for e in b]
    assert any(e["type"] == "trace-create" and e["body"]["id"] == "project-x" for e in flat)
...
def test_ids_are_deterministic_for_dedup(tmp_path, sink):
    p = tmp_path / "project.log"
    p.write_text(CLAUDE_LINE + "\n")
    tracing.Tracer().tick("project-x", str(p))
    first = [e["id"] for b in sink for e in b]
    sink.clear()
    tracing.Tracer().tick("project-x", str(p))                         # fresh tracer = restart
