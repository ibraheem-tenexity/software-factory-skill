...
def test_stage1_and_2_workspace_has_playwright_and_exa_no_deploy_mcps(tmp_path):
    # Stages 1-2 get playwright + exa + memory (web search & project memory, all stages) but NOT
    # the deploy MCPs (railway). openrouter removed SOF-158 (never-called MCP server; the real
    # OpenRouter usage is research.py's direct REST call, not this MCP).
    runs = tmp_path / "runs"; runs.mkdir()
    skills_dir = _make_skills_dir(tmp_path); phase_dir = _make_phase_dir(tmp_path)
    for stage in (1, 2):
        ws = prepare_workspace(str(runs), "project-s%d" % stage, stage,
                               skills_dir=skills_dir, phase_dir=phase_dir)
        mcp = json.loads(open(os.path.join(ws, ".mcp.json")).read())["mcpServers"]
        assert set(mcp) == {"playwright", "exa", "memory"}, \
            "stages 1-2 = playwright + exa + memory, no deploy MCPs, no openrouter"
