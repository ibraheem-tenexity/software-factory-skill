"""Thin console read-accessors backing the Project View: agents(rid), artifacts(rid), run_created(rid)."""
...
def _console(tmp_path):
    return Console(str(tmp_path), launch=FakeLauncher(), new_id=lambda: "run-xyz")


def test_console_artifacts_lists_recorded(tmp_path):
    c = _console(tmp_path)
    rid = c.start_run(RunRequest(description="app", target="railway"))
    RunDB(c._paths(rid)["db"]).record_artifact("Architecture", "workspace/ARCHITECTURE.md",
...
def test_console_run_created_from_first_phase(tmp_path):
    c = _console(tmp_path)
    rid = c.start_run(RunRequest(description="app", target="railway"))
    RunDB(c._paths(rid)["db"]).set_phase("provision", "active")
    assert (c.run_created(rid) or 0) > 0
